...
## Interactive Playground: File operations
Why this matters: command effects are easier to learn when you can see before/after filesystem changes.
What to try: Switch between `cp`, `mv`, `rm`, and `mkdir`, then compare the tree diff after each action.

import FileManipulationPlayground from "./components/file-manipulation/FileManipulationPlayground.jsx"

<FileManipulationPlayground />
