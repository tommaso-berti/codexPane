...
import NodeModuleSelectorPlayground from "./components/imported-modules/NodeModuleSelectorPlayground.jsx"
...
With the
  ...
  module saved to the
     os
  variable, we can access methods such as:
* type - returns the computer's operating system
* arch - returns the operating system [CPU](https://developer.mozilla.org/en-US/search?q=CPU) architecture
...
If the user with the specified ID is found, the callback function is executed with null passed in as the argument for the error parameter, and an object containing the returned user information is passed in as an argument for the user parameter.
...
### alloc
The
     .alloc() method creates (or allocates) a new
     Buffer
...
* encoding (optional): the text encoding that should be used to interpret the fill [argument](https://developer.mozilla.org/en-US/search?q=argument) when it is a [string](https://developer.mozilla.org/en-US/search?q=string) value. defaults to 'utf-8'
...
### toString
The
     .toString() method translates the
     Buffer
  object into a human-readable string. It accepts three optional arguments:
...
As with arrays, the elements of
     Buffer
  objects are zero-indexed, incrementing by
     1
  - we can use these indices to specify the
     start
  and
     end
  values. The default
  ...
  values will result in the entire
     Buffer
  object being transformed into a string.
...
### from
he
     .from() method is provided to create a new
     Buffer
  object from the specified string, array, or buffer. The
     .from() method has several different signatures for the various input types it can take, but we will focus on the [signature for creating a buffer from a string](https://nodejs.org/api/buffer.html#static-method-bufferfromstring-encoding), which accepts two arguments:
...
### concat
The
     .concat() method joins an array of
     Buffer
  objects into a single
     Buffer
  object.
     .concat() comes in handy because a
     Buffer
  object can't be resized. This method accepts two arguments:
...
In the back-end, however, it is essential to interact with the file system in a less restricted way. The Node fs core module is an API for interacting with the **f**ile **s**ystem. It was modeled after the [POSIX](https://en.wikipedia.org/wiki/POSIX) standard for interacting with the file system.

     fs
  provides multiple APIs for handling file data in different ways:
...
* We define an error-first callback function that expects an error to be passed as the first [argument](https://developer.mozilla.org/en-US/search?q=argument) and data as the second. If an error is present, the function will print Something went wrong: $&#123;err&#125; - otherwise, it will print Provided file contained: $&#123;data&#125;.
* We invoke the .readFile() method with three arguments:
	* The first argument is a [string](https://developer.mozilla.org/en-US/search?q=string) that contains a path to the file file.txt.
	* The second argument is a string specifying the file's [character encoding](https://en.wikipedia.org/wiki/Character_encoding) (usually ‘utf-8' for text files). Note that while this [parameter](https://developer.mozilla.org/en-US/search?q=parameter) is optional, the default value is null, which yields the data as a Buffer. To get the data as text, we must specify 'utf-8'.
...
let data;
try {
  data = readFileSync('./file.txt', 'utf-8') console.log(`Provided file contained: ${data}`);
} catch (err) {
  console.log(`Something went wrong: ${err}`);
}
...
```
const fs = require('fs') const fileStream = fs.createWriteStream('output.txt');
...
When writing JavaScript that runs in Node.js, it is sometimes useful to collect **input directly from the user**, in the same way that the browser's
     prompt() function works. Unlike browsers, however, **Node.js does not natively provide a prompt API**.
To allow synchronous user input, we can use the third-party module
     **prompt-sync**
 .

     prompt-sync
  is a small utility that provides a **synchronous** prompt function for Node, allowing programs to pause and wait while the user types something into the terminal. This makes it especially helpful for command-line games, interactive scripts, or situations where asynchronous input (like
     readline
 ) is not ideal.
...
With
     sigint: true
 , pressing **Ctrl+C** stops the script instead of printing
     ^C
 .

### How the prompt function works
Once imported,
     prompt() behaves similarly to the browser's
     prompt()
 :
...
## Interactive Playground: Node Core Module Selector

**Why this matters**  
Node ships many modules, but picking the wrong one leads to unnecessary custom code. This simulator maps common tasks to the right built-in module quickly.

**What to try**  
Choose different backend tasks and compare expected outcomes and code snippets for `fs`, `path`, `events`, and `util`.

<NodeModuleSelectorPlayground />
