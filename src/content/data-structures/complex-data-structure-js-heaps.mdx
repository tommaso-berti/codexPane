...
A heap data structure is commonly implemented as a [binary](https://en.wikipedia.org/wiki/Special:Search?search=binary) tree.
...
The heap array starts with
     null
  so indexing starts at **1** instead of **0**.
Example heap:
...
## bubbleUp() Restores heap property after insertion.
...
## heapify() Restores heap property **from the root downward**.
Steps:
...
## canSwap() Determines if a swap is needed.
Conditions:
