...
An
     Array
  is useful for storing **ordered data**. You can access elements by index:
...
* .pop()
* .sort()
* .includes() However, checking whether an element exists (for example using
     .includes()
 ) may require iterating through the entire array. With very large datasets (e.g., 10,000,000 elements), this can become slow.

## Example: Storing and Checking Numbers (Using an Array) Suppose we receive numbers one at a time and want to:
1. Store each number
...
This works, but
     .includes() may be slow for large arrays because it checks elements sequentially.
...
* storeNumber()
* doYouHaveThisNumber() Only the **underlying implementation** changes.
...
You do not need to know how
     .push() works internally. The array API abstracts away the implementation details.
...
JavaScript does not strictly enforce public/private access, but developers often prefix internal properties with an underscore
     _
  to signal they are not meant for external use.

## Example: Implementing a Stack
A **Stack** is a data structure that allows:
* Adding elements to the top (push)
* Removing elements from the top (pop) Although an array can behave like a stack, arrays also allow behaviors that a stack should not (like accessing by index or adding to the beginning).
### Basic Stack Class
...
The internal array is stored as
     _array
  to indicate it is not meant to be accessed directly.
