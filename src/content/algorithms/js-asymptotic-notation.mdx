---
title: Js Asymptotic Notation
order: 4
---

# Js Asymptotic Notation
...
Important detail:
* findMax() itself is **O(n)**Since
     findMax() is called **inside a loop that runs n times**, we get:
...
## Queue (FIFO - First In First Out) Removing the first inserted element:
...
## Stack (FILO - First In Last Out) Removing the first inserted element requires reaching the **bottom of the stack**.
...
## Open Addressing (Linear Probing) If a collision occurs, the algorithm searches the **next available index**.
Worst case:
