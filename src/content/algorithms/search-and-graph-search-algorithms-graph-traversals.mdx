...
Using [graphs](https://en.wikipedia.org/wiki/Special:Search?search=graphs) to model complex networks is pretty swell, but one way that graphs can really come in handy is with *graph search* algorithms. You can use a graph search [algorithm](https://en.wikipedia.org/wiki/Special:Search?search=algorithm) to traverse the entirety of a graph data structure in search of a specific vertex value.
...
So how do you calculate the [runtime](https://en.wikipedia.org/wiki/Special:Search?search=runtime) for graph search algorithms?
