---
title: Js Binary search trees
order: 15
---

# Js Binary search trees

**A binary tree is an efficient data structure for fast data storage and retrieval due to its** O(log N), [runtime](https://en.wikipedia.org/wiki/Special:Search?search=runtime). It is a specialized tree data structure that is made up of a root node and at most two child branches or subtrees. Each child node is itself a [binary](https://en.wikipedia.org/wiki/Special:Search?search=binary) tree.
...
## Inserting a Value
When inserting a new value into a [binary](https://en.wikipedia.org/wiki/Special:Search?search=binary) tree, we compare it with the root node's value:
...
Recall that a [binary search tree](https://en.wikipedia.org/wiki/Special:Search?search=binary+search+tree) provides a fast and efficient way to store and retrieve values. Like with
     .insert()
...
The base case for our recursive [method](https://en.wikipedia.org/wiki/Special:Search?search=method) is that when the values match, we return the current node. The recursive step is to call itself from an existing left or right child node with the value.
...
To retrieve 75, the [algorithm](https://en.wikipedia.org/wiki/Special:Search?search=algorithm) would proceed as follows:
...
We begin by traversing the left subtree at each level, which brings us to 8, 10, and 11 reside. The [inorder traversal](https://en.wikipedia.org/wiki/Special:Search?search=inorder+traversal) would be:
