...
## DRYing Routes With app.use() Middleware is code that executes between a server receiving a request and sending a response. It operates on the boundary, so to speak, between those two HTTP actions.
...
## next() Most of Express's functionality is chaining middleware. This chain of middleware is referred to as the **middleware stack**.
...
## Open-Source Middleware: Logging (Morgan) There is a huge ecosystem of JavaScript packages that will solve so many of the problems that developers frequently run into.
