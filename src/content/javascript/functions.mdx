...
const functionName1 = paramOne => {};   // one parameter (no parens needed) const functionName2 = (a, b) => {};     // two or more parameters

// Single line block (implicit return) const dupliateNumber = number => number + number;
...
## Interactive Playground: Function Forms Equivalence
**Why this matters:** the same behavior can be expressed with different function syntaxes.

**What to try:** switch function form and verify the same input returns the same result.

import FunctionFormsPlayground from "./components/functions/FunctionFormsPlayground.jsx"

<FunctionFormsPlayground />

## Interactive Playground: Callback Invocation Trace
**Why this matters:** higher-order functions depend on passing callback references correctly.

**What to try:** choose a callback and observe each invocation step in the trace.
...
// add_one() still has access to the argument from curried_add() console.log(add_one(10)); // 11
~~~
