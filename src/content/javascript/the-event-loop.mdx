...
- Queues of pending work (tasks/events) It moves ready-to-run callbacks from queues to the call stack when the stack is empty.
...
## Interactive Playground: Event Loop Ordering
**Why this matters:** understanding queue order prevents timing bugs in asynchronous code.

**What to try:** run each scenario and compare sync, microtask, and macrotask ordering.
...
## Interactive Playground: Memory Reachability
**Why this matters:** memory leaks happen when objects stay reachable longer than intended.

**What to try:** toggle references A/B and observe when the object becomes collectible.
