...
~~~js
const cfg = { mode: "dev" };
cfg.mode = "prod";    // ok (object mutated) console.log(cfg.mode); // "prod"
~~~
...
function f() {
    ...
    // console.log(b); // ReferenceError (let not initialized yet) var a = 1;
    let b = 2;
}
f();
~~~
...
## Hoisting
During the compilation phase of code execution, the JavaScript engine allocates memory to save the names of declared variables and functions by **hoisting** variable and function declarations to the top of their current scope. Hoisting does not actually move your code around; instead, it:
...
`let` and `const` differ from `var` in how they initialize as well; while the names are hoisted, they are **not initialized**. So calling a variable in your code **before it is initialized** results in a `ReferenceError` (temporal dead zone).
...
// let/const: name hoisted, NOT initialized (TDZ) try {
...
try {
  c = 3;          // ReferenceError (assignment before initialization) const c = 3;
} catch (e) {
  console.log(e.name); // "ReferenceError"
}
~~~

> **Note:** This stricter behavior is one reason `let`/`const` are preferable—fail fast instead of silently reading `undefined`.

## Interactive Playground: Scope and Hoisting Checks
**Why this matters:** scope and reassignment mistakes cause subtle runtime bugs.

**What to try:** switch declaration type, toggle block scope, then run and compare output/errors.

import ScopePlayground from "./components/variables/ScopePlayground.jsx"

<ScopePlayground />

> **Tip:** Toggle between `var`, `let`, and `const` and try enabling a block to see how scoping and redeclaration rules affect the output and errors.
