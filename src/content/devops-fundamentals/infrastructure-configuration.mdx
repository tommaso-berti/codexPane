...
In today’s [cloud](https://developer.mozilla.org/en-US/search?q=cloud) world, server provisioning means spinning up a virtual machine. There are other types of provisioning as well:
* **Network provisioning** means setting up network components such as switches, [routers](https://developer.mozilla.org/en-US/search?q=routers), and gateways.
...
* Updating to a specific [Linux](https://developer.mozilla.org/en-US/search?q=Linux) distribution.
* Setting up logging.
* Creating [database](https://developer.mozilla.org/en-US/search?q=database) configuration files.
...
Realizing the flaws with manual configuration, teams began to automate tasks. At first, this consisted of shell scripts that configured servers. Tools such as [csshx](https://github.com/brockgr/csshx) allowed commands to be passed to many servers simultaneously.
...
One example of a configuration orchestration tool is [Terraform](https://www.terraform.io/). It has native support for the most common cloud providers. Configuration files are written in either HashiCorp Configuration Language (HCL) or [JavaScript Object Notation (JSON)](https://developer.mozilla.org/en-US/search?q=JavaScript+Object+Notation+%28JSON%29). These files are then passed into Terraform. Terraform makes the cloud [API](https://developer.mozilla.org/en-US/search?q=API) calls needed to spin up the declared resources.
Configuration management tools include [Ansible](https://www.ansible.com/), [Chef](https://www.chef.io/), and [Puppet](https://puppet.com/). These tools maintain a consistent state across cloud resources. They can help automate daily tasks. Examples of these tasks are:
...
Below we have an Ansible “play” which is written in [YAML](https://yaml.org/). Ansible will execute tasks in the order in which they appear in the play.
