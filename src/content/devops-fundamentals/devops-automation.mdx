...
![B77CBF08-8109-4871-8A62-02A1B14737DC](images/B77CBF08-8109-4871-8A62-02A1B14737DC.png)
### Benefits
...
## Pattern: background job deploy with systemd and github actions
Some automation workloads are not web apps. A common pattern is a scheduled job (for example: report generation, notifications, data sync) deployed to a VPS and run by `systemd` timers.
...
sudo mkdir -p /opt/<job-name>/app /opt/<job-name>/run
sudo chown -R deploy:deploy /opt/<job-name>
~~~

### Service and timer units
~~~ini
# /etc/systemd/system/<job-name>.service
[Unit]
Description=Background automation job
...
User=deploy
Group=deploy
WorkingDirectory=/opt/<job-name>/app
EnvironmentFile=/opt/<job-name>/.env
ExecStart=/opt/<job-name>/app/.venv/bin/python /opt/<job-name>/app/main.py
~~~

~~~ini
# /etc/systemd/system/<job-name>.timer
[Unit]
Description=Run automation job daily
...
sudo systemctl enable --now <job-name>.timer
~~~

### CI deploy essentials
- Keep credentials in repository secrets (for example `VPS_HOST`, `VPS_USER`, `VPS_SSH_KEY`).
- Pull latest code on the VPS, refresh virtualenv dependencies, write `.env`, then restart or run the service.
- Validate with:
~~~bash
sudo systemctl start <job-name>.service
journalctl -u <job-name>.service -n 100 --no-pager
systemctl list-timers | grep <job-name>
~~~

> Tip: allow only specific `systemctl` commands via sudoers instead of full passwordless sudo.
