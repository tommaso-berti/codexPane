...
## Interactive Playground: Async Await Error Guard
**Why this matters:** `try/catch` alone is not enough if you skip `response.ok` checks.

**What to try:** run the same scenario with and without the `response.ok` guard and compare the final flow.
