...
> Attention: Prefer a final `.catch(err => …)` to handle both network and thrown application errors in one place.
...
## Interactive Playground: Fetch Promise Chain Flow
**Why this matters:** `.then()` chains are easy to misuse when HTTP errors and network errors are mixed.

**What to try:** walk through each step for success, HTTP error, and network error scenarios.
