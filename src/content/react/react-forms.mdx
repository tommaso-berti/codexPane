...
const [name, setName] = useState('') const [phone, setPhone] = useState('') const [address, setAddress] = useState('') const [order, setOrder] = useState('') const handleSubmit = (e) => {
  e.preventDefault();
...
## Interactive Playground: Controlled Forms

**Why this matters:** form UX quality depends on clear controlled state and predictable validation.

**What to try:** type valid/invalid inputs, check status changes, then run submit simulation.
