...
- **Domain** (e.g., `example.com`) — specifies the website that hosts the resource. The domain serves as the entry point for your application.
...
React Router provides multiple routers; the most common is `createBrowserRouter`. (Other options and trade offs are outside the scope of this lesson—see the React Router docs to explore them.) Import and initialize:
...
- Example: `https://www.google.com/search?q=react+router`
...
## Interactive Playground: Route Matching

**Why this matters:** route correctness depends on how pathname and query state are combined.

**What to try:** edit path and query, run navigation, then inspect matched route and history trace.
