...
const logger = storeAPI => next => action => {
  console.log(storeAPI.getState()) const nextState = next(action) console.log(nextState) return nextState;
};
...
console.log(remindMeTo('call mom')) const reminder = remindMeLater('get groceries') console.log(reminder())
~~~
...
const fetchUserById = createAsyncThunk(
    ...
    async (userId, thunkAPI) => { // payload creator
        const response = await fetchUser(userId) return response.data
    }
)
~~~
...
## Interactive Playground: Thunk Lifecycle Pipeline

**Why this matters:** middleware/thunk debugging depends on understanding lifecycle action order.

**What to try:** run fulfilled and rejected scenarios and compare deterministic lifecycle traces.
