...
> **Note**
> In modern Redux apps, `createStore` is considered legacy. New code typically uses **@reduxjs/toolkit**’s `configureStore`. The examples below keep `createStore` to match the text and keep concepts focused.
...
## Interactive Playground: Store Dispatch Cycle

**Why this matters:** Redux APIs are simple, but understanding their order (`dispatch`, reducer, `subscribe`, `getState`) is essential.

**What to try:** switch action mode, run dispatches, and inspect event log/state snapshots.
