...
~~~
/* todosSlice.js */
// Configuration object for createSlice() const options = {
...
const todosSlice = createSlice({
  ...
  reducers: {
    ...
    toggleTodo: (state, action) => {
      const todo = state.find(todo => todo.id === action.payload.id) if (todo) {
...
const todosSlice = createSlice({
  ...
  reducers: {
    ...
    toggleTodo(state, action) {
      const todo = state.find(todo => todo.id === action.payload) if (todo) {
        todo.completed = !todo.completed
      }
    }
  }
...
## Interactive Playground: Slice Actions and Filtering

**Why this matters:** RTK reduces boilerplate, but reducer intent and derived views still need to be explicit.

**What to try:** add todos, change filter, then toggle the first visible item and inspect output.
