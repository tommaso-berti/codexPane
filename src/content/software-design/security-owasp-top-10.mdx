...
Here is a malicious query that searches for the product “soap” while attaching a
     UNION
  command to steal usernames and passwords:
...
Unlike injection, where the malicious input is usually from an input field, XXE involves an attacker uploading a maliciously crafted [XML](https://owasp.org/www-community/) file. XML is a markup language that supports potentially insecure features, and if a website is using an XML processor with those features enabled, an attacker can use XXE to wreak havoc. In the worst case, the attacker may be able to execute arbitrary code - just about the worst case scenario for security.
![C51625F1-DC99-4BBC-878D-F9FCC20862D1](images/C51625F1-DC99-4BBC-878D-F9FCC20862D1.png) 1. The first part, which says ``<!ENTITY xxe SYSTEM "file:///etc/passwd" >]``, defines an entity pointing to the `etc` password file.
